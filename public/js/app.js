...
var conversationsCount = $('.conversation').length; // Count of conversations window opened
var messaging = $('#messaging'); // Panel
var messageTemplate = $('#message-template'); // Template of 
var loggedUserName = $('#self-contact span').html(); // Username of logged user
var canOpenConversation = true; // Flag to block open same conversation
var canSendMessage = true; // Flag to block multiple same messages
...
$('body').on('click', '.ajax-link', function (e) {
	...
	if (link.hasClass('contact') || link.hasClass('conversation-new')) {
		var convElement = link.data('conv-id');
		var id = link.data('conv-id').split('-')[1];
		$("#modal").modal('hide');
		if ($(convElement).length === 0 && canOpenConversation) {
			$.ajax({
				...
				var messages = $(convElement).find('.messages');
...
$('body').on('submit', '.ajax-form', function (e) {
	...
	if (form.hasClass('message-form')) {
		var input = form.find('.message-input');
		...
		if (input.val() && canSendMessage) {
			$.ajax({
				...
				success: function success(data) {
					...
					var msg = newMessage(input.val(), loggedUserName, data.created_at);
					input.val('');
					messages.animate({ scrollTop: messages[0].scrollHeight });
					messages.append(msg);
					canSendMessage = true;
		...
		var _input = form.find('.form-control');
